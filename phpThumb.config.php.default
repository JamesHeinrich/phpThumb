...
$PHPTHUMB_CONFIG['config_high_security_url_separator'] = '&';    // should almost always be left as '&'. Must be a single character. Do not change to '&amp;' -- htmlspecialchars wrapped around phpThumbURL() takes care of this without breaking the hash
...
function phpThumbURL($ParameterString, $path_to_phpThumb='phpThumb.php') {
	...
	return $path_to_phpThumb.'?'.$ParameterString.$PHPTHUMB_CONFIG['config_high_security_url_separator'].'hash='.md5($ParameterString.$PHPTHUMB_CONFIG['high_security_password']);
}
